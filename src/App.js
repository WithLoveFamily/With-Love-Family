    ...
    <div>
      ...
      {SECTIONS.map(sec=>(
        <div key={sec.key} style={{ marginBottom:16, background:C.gray, borderRadius:8, padding:12 }}>
          ...
          {NOTE_SECTIONS.includes(sec.key) && (
            <NoteRow data={data} onChange={onChange} sectionKey={sec.key} dayKey={dayKey} isConsultant={isConsultant} />
          )}
        </div>
      ))}
        ...
        <tbody>
          {rows.map((row,i)=>{
            if(row.type==="header") {
              return (
                <tr key={i}>
                  <td colSpan={50} style={{ background:headerBg[row.key]||C.blue, color:"#fff", fontWeight:700, padding:"6px 10px", fontSize:13, border:"1px solid #e0e0e0" }}>{row.label}</td>
                </tr>
              );
            }
      ...
      <div style={{ padding:16 }}>
        ...
        <div style={{ background:C.goldLight, borderRadius:8, padding:16, marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <div style={{ flex:1, minWidth:140 }}>
              <label style={{ fontSize:12 }}>Nome papa</label>
